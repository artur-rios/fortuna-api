...
| **OpenTelemetry.Extensions.Hosting** (+ `.Instrumentation.AspNetCore`, `.Instrumentation.Http`, `.Exporter.Prometheus.AspNetCore`) | 1.18.0; the exporter 1.18.0-beta.1 | WebApi | Metrics for the hosted API, scraped by Prometheus on a private port. The Prometheus exporter has only ever shipped as a beta; it is pinned to the build matching the stable SDK. |
