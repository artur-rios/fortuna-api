...
| Heimdall exchange | `FORTUNA_HEIMDALL_BASE_URL`, `FORTUNA_HEIMDALL_SCOPE_ID` | Required base URL and non-empty public scope identifier used only by the explicit `api/auth` credential exchanges. |
...
| Processing consent | `FORTUNA_CONSENT_EXTERNAL_PROCESSING_VERSION` | Current hosted `external-data-processing` disclosure version. Defaults to `1.0`; changing it makes older decisions non-current until explicitly granted again. |
...
| Health | `FORTUNA_HEALTH_JOB_MAX_PENDING_SECONDS` | Maximum age of the oldest pending job before the job runner is reported unhealthy; defaults to `300`. |
