...
The given project `ArturRios.Fortuna.WebApi` has no vulnerable packages given the current sources.
The given project `ArturRios.Fortuna.Domain` has no vulnerable packages given the current sources.
"""
...
The given project `ArturRios.Fortuna.WebApi` has the following vulnerable packages
   [net10.0]:
...
class ParseTests(unittest.TestCase):
    ...
    def test_given_a_top_level_vulnerability_when_parsed_then_its_columns_are_read(self):
        ...
        self.assertEqual("ArturRios.Fortuna.WebApi", finding.project)
