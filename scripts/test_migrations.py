...
class ParseEnvFileTests(unittest.TestCase):
    def test_given_quoted_values_when_parsed_then_quotes_are_stripped(self):
        parsed = parse_env_file('FORTUNA_DATA_DATABASETYPE="PostgreSql"\n')

        self.assertEqual({"FORTUNA_DATA_DATABASETYPE": "PostgreSql"}, parsed)
    ...
    def test_given_a_value_containing_equals_when_parsed_then_the_value_is_intact(self):
        parsed = parse_env_file('CONN="Host=localhost;Database=fortuna"\n')

        self.assertEqual({"CONN": "Host=localhost;Database=fortuna"}, parsed)
