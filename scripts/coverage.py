...
project was renamed from `identity-manager-api` to `fortuna-api`; `dotnet build` only overwrites
...
SOLUTION = REPO_ROOT / "src/ArturRios.Fortuna.sln"
...
def collect(test_filter):
    ...
    command = ["dotnet", "test", str(SOLUTION), "-m:1", "--collect:XPlat Code Coverage"]
...
def main():
    ...
    print("Preview it with: hugo -s docs server  ->  http://localhost:1313/fortuna-api/coverage-report/")
